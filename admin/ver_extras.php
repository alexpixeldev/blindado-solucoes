...
// Buscar extras
$stmt = $conn->prepare("
    SELECT e.*, u.nome as nome_registrador 
    FROM extras e 
    LEFT JOIN usuarios u ON e.registrado_por = u.id 
    WHERE e.usuario_id = ? 
    ORDER BY e.data_extra DESC, e.hora_inicio ASC
");
$stmt->bind_param("i", $id);
$stmt->execute();
...
<head>
    ...
    <title>Histórico de Extras | Blindado Soluções</title>
            ...
            <main class="flex-1 overflow-y-auto p-4 sm:p-8 custom-scrollbar">
                ...
                <div class="mb-8 animate-fade-in flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h1 class="text-2xl font-bold text-slate-900 sm:text-3xl">Histórico de Extras</h1>
                    ...
                    <div class="flex gap-3">
                        <a href="registrar_extra.php?id=<?= $id ?>" style="display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;border:none;border-radius:3px;background:#22c55e;color:#fff;font-size:10px;cursor:pointer;padding:0;line-height:1;flex-shrink:0;text-decoration:none" title="Novo Extra"><i class="fas fa-plus" style="font-size:10px"></i></a>
                    </div>
                </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-50 border-b border-slate-100">
                                    ...
                                    <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Horário</th>
                                    <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Local</th>
                                    <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Registrado por</th>
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center justify-center text-slate-400">
                                                ...
                                                <p class="text-sm font-medium">Nenhum extra registrado para este colaborador.</p>
                                            </div>
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($extras as $e): ?>
                                        <tr class="hover:bg-slate-50/50 transition-colors group">
                                            <td class="px-6 py-4">
                                                <span class="text-sm font-bold text-slate-700"><?= date('d/m/Y', strtotime($e['data_extra'])) ?></span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span class="text-xs font-medium text-slate-600"><?= date('H:i', strtotime($e['hora_inicio'])) ?> - <?= date('H:i', strtotime($e['hora_fim'])) ?></span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span class="text-sm text-slate-600"><?= htmlspecialchars($e['local']) ?></span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span class="text-xs text-slate-500"><?= htmlspecialchars($e['nome_registrador'] ?? '-') ?></span>
                                            </td>
                                            <td class="px-6 py-4">
                                                <div class="flex items-center justify-center gap-2">
                                                    ...
                                                    <?php endif; ?>
                                                    <a href="editar_extra.php?id=<?= $e['id'] ?>" style="display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;border:none;border-radius:3px;background:#94a3b8;color:#fff;font-size:10px;cursor:pointer;padding:0;line-height:1;flex-shrink:0;text-decoration:none" title="Editar"><i class="fas fa-edit" style="font-size:10px"></i></a>
                                                </div>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
