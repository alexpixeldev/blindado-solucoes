...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nome) {
        $stmt = $conn->prepare("INSERT INTO administradoras (nome, telefone, email) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $nome, $telefone, $email);
        
        if ($stmt->execute()) {
            $_SESSION['mensagem'] = "Administradora cadastrada com sucesso!";
            $_SESSION['mensagem_tipo'] = "success";
            header("Location: edificios.php?tab=administradoras");
            exit();
        } else {
            $erro = "Erro ao cadastrar administradora: " . $conn->error;
        }
        $stmt->close();
    } else {
...
<head>
    ...
    <title>Cadastrar Administradora | Blindado Soluções</title>
            ...
            <main class="flex-1 overflow-y-auto p-4 sm:p-8 custom-scrollbar">
                ...
                <div class="mb-8 animate-fade-in">
                    <div class="flex items-center gap-4">
                        <a href="edificios.php?tab=administradoras" style="display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;border:none;border-radius:3px;background:#94a3b8;color:#fff;font-size:10px;cursor:pointer;padding:0;line-height:1;flex-shrink:0" title="Voltar"><i class="fas fa-arrow-left" style="font-size:10px"></i></a>
                        <div>
                            <h1 class="text-2xl font-bold text-slate-900 sm:text-3xl">Cadastrar Administradora</h1>
                            <p class="mt-1 text-slate-500">Adicione uma nova administradora de condomínios ao sistema.</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="admin-card">
                        <form method="POST" class="space-y-6">
                            <div class="space-y-2">
                                ...
                                <input type="text" name="nome" class="form-input" required value="<?php echo htmlspecialchars($_POST['nome'] ?? ''); ?>" placeholder="Ex: Administradora Central">
                            </div>
                            ...
                            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <input type="email" name="email" class="form-input" value="<?php echo htmlspecialchars($_POST['email'] ?? ''); ?>" placeholder="contato@administradora.com">
                                </div>
                            </div>
