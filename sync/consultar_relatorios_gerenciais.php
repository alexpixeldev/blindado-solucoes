...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_relatorio'])) {
    if ($usuario_categoria === 'gerente') {
        ...
        $stmt = $conn->prepare("SELECT descricao FROM relatorios_gerenciais WHERE id = ?");
        $stmt->bind_param("i", $id_delete);
        $stmt->execute();
        $stmt->bind_result($descricao);
        if ($stmt->fetch()) {
            preg_match_all('/uploads\/relatorios_gerenciais\/([^"\'\\s<>]+)/', $descricao, $matches);
            foreach ($matches[1] as $media) {
                $dir_upload = realpath(__DIR__ . '/../uploads/relatorios_gerenciais/');
                if ($dir_upload) {
        ...
        $stmt = $conn->prepare("DELETE FROM relatorios_gerenciais WHERE id = ?");
        $stmt->bind_param("i", $id_delete);
        $stmt->execute();
        $stmt->close();
        header("Location: consultar_relatorios_gerenciais.php");
        exit();
    }
}
...
if ($data_filtro) $query .= " AND o.data_relatorio = '$data_filtro'";
if ($local_filtro) {
...
$query .= " ORDER BY o.data_relatorio DESC, o.id DESC";
$result = $conn->query($query);
$relatorios_raw = $result ? $result->fetch_all(MYSQLI_ASSOC) : [];

$livro_relatorios = [];
foreach ($relatorios_raw as $row) {
    $key = $row['data_relatorio'] . '_' . $row['usuario_id'];
    if (!isset($livro_relatorios[$key])) {
        $livro_relatorios[$key] = ['info' => $row, 'registros' => []];
    }
    $livro_relatorios[$key]['registros'][] = $row;
}
...
<head>
    ...
    <title>Consultar Relatórios Gerenciais | Blindado Soluções</title>
            ...
            <main class="flex-1 overflow-y-auto p-4 sm:p-8 custom-scrollbar">
                <div class="max-w-5xl mx-auto">
                    <div class="mb-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 animate-fade-in">
                        <div>
                            <h1 class="text-3xl font-bold text-slate-900">Relatórios Gerenciais</h1>
                            <p class="text-slate-500">Histórico de relatórios gerenciais.</p>
                        </div>
                        <a href="registrar_relatorio_gerencial.php" class="icon-btn-green" title="Novo Registro"><i class="fas fa-plus" style="font-size:10px"></i></a>
                    </div>
                    ...
                    <?php else: ?>
                        <div class="space-y-6 animate-slide-up">
                            <?php foreach ($livro_relatorios as $key => $relatorio): $meta = $relatorio['info']; ?>
                                <div class="plantao-card">
                                    <div class="plantao-header" onclick="toggleDetails('<?= $key ?>')">
                                        <div class="flex items-center gap-4">
                                            <div class="h-12 w-12 rounded-2xl bg-indigo-50 text-indigo-500 flex items-center justify-center shadow-sm">
                                                <i class="fas fa-file-alt text-xl"></i>
                                            </div>
                                            <div>
                                                <h3 class="font-bold text-slate-900 text-lg">Relatório <?= date('d/m/Y', strtotime($meta['data_relatorio'])) ?></h3>
                                    ...
                                    <div id="details-<?= $key ?>" class="hidden overflow-hidden transition-all duration-500">
                                        <?php foreach ($relatorio['registros'] as $reg): ?>
                                            <div class="relatorio-body border-b border-slate-50 last:border-0">
                                                <div class="flex justify-between items-start mb-8">
                                                    ...
                                                    <?php if ($reg['usuario_id'] == $_SESSION['usuario_id'] || $usuario_categoria === 'gerente'): ?>
                                                        <div class="flex gap-2">
                                                            <a href="editar_relatorio_gerencial.php?id=<?= $reg['id'] ?>" class="icon-btn" title="Editar"><i class="fas fa-edit" style="font-size:10px"></i></a>
                                                            <?php if ($usuario_categoria === 'gerente'): ?>
                                                                <form method="POST" onsubmit="return confirm('Excluir este registro?');" class="inline">
                                                                    ...
                                                                    <button type="submit" name="delete_relatorio" class="icon-btn-red" title="Excluir"><i class="fas fa-trash-alt" style="font-size:10px"></i></button>
                                                                </form>
                                                            <?php endif; ?>
                                                        </div>
                                                    <?php endif; ?>
                                                ...
                                                <div class="prose max-w-none">
                                                    <?php 
                                                        $desc = $reg['descricao'];
                                                        $desc = preg_replace('/@(\d{2}\/\d{2}\/\d{4})/', '<a href="consultar_relatorios_gerenciais.php?data=' . date('Y-m-d', strtotime(str_replace('/', '-', '$1'))) . '" class="mention">@$1</a>', $desc);
                                                        echo nl2br($desc); 
                                                    ?>
                                                </div>
                                            </div>
