...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $check = @$conn->query("SHOW COLUMNS FROM edificios LIKE 'retirada_lixo'");
    if (!$check || $check->num_rows == 0) {
        @$conn->query("ALTER TABLE edificios ADD COLUMN retirada_lixo TINYINT(1) DEFAULT 0 AFTER requer_selfie");
    }

    $stmt = $conn->prepare("UPDATE edificios SET retirada_lixo = ? WHERE id = ?");
