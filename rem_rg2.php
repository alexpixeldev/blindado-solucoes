...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['gerente_id'])) {
    ...
    $stmt = $conn->prepare("SELECT id FROM usuarios WHERE id = ? AND categoria IN ('gerente', 'gerente_monitoramento', 'gerente_zeladoria', 'gerente_tecnica')");
...
// Lista de gerentes
$gerentes = $conn->query("SELECT id, nome, nome_real, whatsapp, gerente_ronda FROM usuarios WHERE categoria IN ('gerente', 'gerente_monitoramento', 'gerente_zeladoria', 'gerente_tecnica') ORDER BY gerente_ronda DESC, nome_real")->fetch_all(MYSQLI_ASSOC);
